...
class EmbraceNet(nn.Module):
    def __init__(
        ...
        if not bypass_docking:
            for i, input_size in enumerate(input_size_list):
                # Dynamically create the Linear layer and initialize it with Xavier uniform
                layer = nn.Linear(input_size, embracement_size)
                if init_uniform_weights:
                    init.xavier_uniform_(
                        layer.weight
                    )  # Apply Xavier uniform initialization to weights

                    if layer.bias is not None:
                        init.constant_(layer.bias, 0)  # Initialize biases to zero

                # Assign the initialized layer to the model
                setattr(self, f"docking_{i}", layer)

                print(
                    f"Created docking_{i} with input size {input_size} and output size {embracement_size}"
                )
                print(layer.weight)
    ...
    def forward(self, input_list, availabilities=None, selection_probabilities=None):
        ...
        )  # [batch_size, embracement_size, num_modalities]
